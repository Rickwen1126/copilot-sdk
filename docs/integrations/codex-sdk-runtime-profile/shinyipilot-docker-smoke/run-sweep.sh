...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/shinyipilot-codex-docker-sweep-$(date +%Y%m%d-%H%M%S)}"
SWEEP_TIMEOUT_SECONDS="${SWEEP_TIMEOUT_SECONDS:-360}"
SWEEP_TURN_TIMEOUT_SECONDS="${SWEEP_TURN_TIMEOUT_SECONDS:-240}"
TMP_PARENT="${TMPDIR:-/tmp}"
BUILD_CONTEXT="$(mktemp -d "${TMP_PARENT%/}/shinyipilot-codex-sweep.XXXXXX")"
...
docker run --rm \
    --name "shinyipilot-codex-sweep-$(date +%s)" \
    ...
    -e "SHINYIPILOT_DOCKER_MODE=sweep" \
    -e "SWEEP_TIMEOUT_SECONDS=${SWEEP_TIMEOUT_SECONDS}" \
    -e "SWEEP_TURN_TIMEOUT_SECONDS=${SWEEP_TURN_TIMEOUT_SECONDS}" \
    "${IMAGE}"
